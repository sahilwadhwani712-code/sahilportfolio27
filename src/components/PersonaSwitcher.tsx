            ...
            <div className="mb-3 sm:mb-4 min-h-[3.5rem] sm:min-h-[3rem]">
              <AnimatePresence mode="wait">
                <motion.p
                  ...
                  className={`font-body text-[12px] sm:text-[13px] max-w-md leading-[1.6] ${isLight ? "text-stone-700" : "text-stone-300/90"}`}
                >
                  {persona.description}
                </motion.p>
              </AnimatePresence>
              ...
              >
                {(persona.socials ?? []).map((s) => (
                  <a
                    ...
                    aria-label={s.label}
                    title={s.label}
                    className={`group relative inline-flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full border transition-colors ${isLight ? "border-stone-500/30 hover:border-stone-700/60" : "border-white/15 hover:border-white/35"}`}
                    style={{
                  ...
                  >
                    ...
                    />
                    <s.icon className={`relative z-10 w-[13px] h-[13px] transition-colors ${isLight ? "text-stone-800 group-hover:text-stone-900" : "text-stone-200 group-hover:text-white"}`} />
                  </a>
                ))}
              </motion.div>
            </AnimatePresence>
              ...
              >
                {persona.stats.map((s, i) => (
                  <motion.div
                    ...
                    className="rounded-lg px-3 py-1.5 cursor-default"
                    style={{
                  ...
                  >
                    <div
                      className="font-display text-sm sm:text-base font-bold leading-none"
                      style={{ color: persona.accent }}
                    >
                      {s.value}
                    </div>
                    <div className={`text-[9px] sm:text-[10px] font-body mt-1 tracking-wide ${isLight ? "text-stone-700" : "text-stone-400"}`}>
                      {s.label}
                    </div>
                  </motion.div>
                ))}
        ...
        {personas.map((p, i) => {
          ...
          return (
            <button
              ...
              title={p.label}
              className="group relative inline-flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full transition-transform hover:scale-105 active:scale-95"
              style={{
                background: isActive
                  ? `${p.accent}`
                  : isLight
            ...
            >
              <Icon
                className="w-[15px] h-[15px] sm:w-4 sm:h-4 transition-colors"
                style={{
                  color: isActive
                    ? "white"
                    : isLight
